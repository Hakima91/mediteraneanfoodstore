...
//RESULTS SECTION
const section = document.querySelector('section'); // Setting a variable called section, checking HTML file for first instance of "section" class, defining variable
nav.style.display = 'none'; // Removing nav bar from page
let pageNumber = 0; // Establishing pageNumber as zero
let displayNav = false; //Establishing a variable called displayNav as false
//1                     //2
...
function fetchResults(e) { //creating a new event function called fetchResults
  e.preventDefault(); //prevents the submit button from submitting the form as default
  ...
  if (startDate.value !== '') { // Says if the start date is not blank
...
function displayResults(json) { // Creates a new function called displayResults
  ...
  while (section.firstChild) { // runs a loop as long as the section has a first child
  ...
  if (articles.length === 0) { // if items in the array is equal to zero
    ...
    for(let i = 0; i < articles.length; i++) { // establishes variables for the f elements for each object inside the array as long as the array has one or more objects
      ...
      let current = articles[i]; //establishes an element inside the loop called current, defines the variable as our article variable
